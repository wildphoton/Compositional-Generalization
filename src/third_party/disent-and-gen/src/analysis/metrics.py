...
class CompGenMetrics:
    def __init__(self, train_data, test_data, n_factors, regressor='lasso',
    ...
    def compute_score(self, model, model_zs=None):
        if model_zs is None:
            train_X, train_y = infer(model, self.train_data)
            train_X, train_y = train_X.numpy(), train_y.numpy()
            test_X, test_y = infer(model, self.test_data)
            test_X, test_y = test_X.numpy(), test_y.numpy()
        else:
            (train_X, train_y), (test_X, test_y) = model_zs

        train_X = (train_X - train_X.mean(axis=0))  # / (X.std(axis=0) + EPS)
        test_X = (test_X - test_X.mean(axis=0))  # / (X.std(axis=0) + EPS)
...
def compute_compgen_metric(models, train_data, test_data):
    ...
    com_gen = CompGenMetrics(train_loader, test_loader, n_factors=n_factors)
