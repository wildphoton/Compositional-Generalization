...
from analysis.utils import load_dataset, load_decoder, load_lgm, safe_save
...
from analysis.metrics import compute_dci_metrics, dci2df, compute_compgen_metric
...
def disentanglement_vs_nll(raw_train, raw_test, end2end_models, gt_decoder,
    ...
    if gt_decoder:
        ...
        train_nll = pd.concat([train_nll, decoder_train_nll])
        test_nll = pd.concat([test_nll, decoder_test_nll])

    score_names = ['Training', 'Test']
    print(train_nll, test_nll)
    recons_metrics = pd.concat([train_nll, test_nll], keys=score_names,
                               names=['Data', 'Models'])
    ...
    if gt_decoder:
        ...
        disent = np.asarray(overall_disent.values)
...
def eval_comp_gen(end2end_models, dataset, model_names):
    raw_train, raw_test = dataset
    factors = raw_train.factors
    factors = list(map(lambda s: s.replace('_', ' '), factors))
    results = compute_compgen_metric(end2end_models, raw_train.get_supervised(), raw_test.get_supervised())
    df = pd.DataFrame(results, index=model_names, columns=factors)
...
def main(dataset, condition, variant, sims_ids, model_names,
    ...
    comp_gen_df = eval_comp_gen(end2end_models, dataset, model_names)
    comp_gen_df.to_csv(results_folder + '{}-compgen.csv'.format(variant))
