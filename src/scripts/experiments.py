...
def train_vae(config, args):
    ...
    if os.path.isfile(ckpoint_path) and config['exp_params']['max_epochs'] == 0 and\
            not (args.overwrite or args.test or args.compmetric):
    ...
    if os.path.isfile(ckpoint_path) and config['exp_params']['max_epochs'] == 0 and not args.notrain:
...
def scikitlearn_eval(config, args):
    if args.finetune:
        ...
        dm = get_datamodule(config['exp_params']['dataset'],
                            ...
                            random_seed=config['exp_params']['random_seed'],
                            in_distribution_test=args.TestInDist,
                            )

        config['eval_params']['inDist'] = args.TestInDist
...
def add_vae_argument(parser):
    ...
    parser.add_argument('--test', '-t', action='store_true',
                        help='test vae models on the pretraining task')
    # parser.add_argument('--notrain', '-ntr', action='store_true',
    #                     help='do not run pretraining')
    parser.add_argument('--finetune', '-ft', action='store_true',
                        help='finetune mode: only train a linear head or GBT on trained vae')
    ...
    parser.add_argument('--project', '-pj', type=str, default='comp_gen',
    ...
    parser.add_argument('--TestInDist', '-id', action='store_true',
                        help='test the performance in training data')