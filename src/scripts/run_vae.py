...
def finetune(config, args):
    if (not args.test or args.finetune) and (not args.nofinetune):
        ...
        if config['eval_params']['train_steps'] > 0:
            ...
            weights = {k.replace('linear_layer', 'task_head'): v for k, v in ft_ckpt['state_dict'].items() if 'backbone' not in k and 'latent' not in k}
...
def main():
    ...
    with open(linear_eval_config_file, 'r') as file:
        try:
            linear_eval_cfg = yaml.safe_load(file)
        except yaml.YAMLError as exc:
            print(exc)
    sklearn_eval_config_file = 'configs/scikitlearn_eval.yaml'
    with open(sklearn_eval_config_file, 'r') as file:
        try:
            sklearn_eval_cfg = yaml.safe_load(file)
        except yaml.YAMLError as exc:
            print(exc)
    ...
    for data in ['dsprites90d']:
        # for gen_type in ['range', ]:
        # for gen_type in ['element', ]:
        # for gen_type in ['multiElement', ]:
        for gen_type in ['random', ]:
            for version in versions[args.version_id]:
                ...
                for seed in (2001, ):
                    # for latent, beta in product((1024, 2048, 4096, 8192, ), (0., )):
                    for recon_loss, beta in product(('mse', ), (1, )):

                        config['model_params']['beta'] = beta
                        config['model_params']['latent_dim'] = 10
                        config['model_params']['recon_loss'] = recon_loss

                        # config['exp_params']['latent_dim'] = config['model_params']['latent_dim']
